...
module.exports = function() {
	...
	app.get("/for-await-of-loops", async (req,res) =>{
		...
		const asyncIterator = {
			next: async() => {
				...
				i++;
				return Promise.resolve({ value: fileText, done: false});
			}
		};
	...
	app.get("/regex2018", (req,res) => {
		...
		output +=  "<h3 style='margin-bottom: 0;'>1. s (dotAll) flag</h3><h4 style='margin-top: 0;'> allows the '.' to match new line characters. Below are the results of using the expression /^.*/ on a string with and without the 's' flag</h4>";
		...
		output +=  "<h3 style='margin-bottom: 0;'>2. Name Captured Groups</h3><h4 style='margin-top: 0;'> allows you to capture sections of a string and store them inside a match object by name:</h4>";
		...
		output += "<p><strong>Year: </strong>" + year +"</p><p><strong>Month: </strong>"+ month +"</p><p><strong>Day: </strong>"+ day + "</p>";
		output += "<h4>Name capture groups also allow for backreferences. Backreferences allow for matching a name captured group making it easier to match repeated patterns:</h4>";
		...
		output += "<h4>Name capture groups can also be used within the replace() method for doing tasks like changing our date string's order:</h4>";
		output += "1997-08-12".replace(RE_DATE, "$<month>/$<day>/$<year>"); //uses name groups to swap the date order
		output += "</ul>";
		
		//START OF LOOKBEHINDS
		output +=  "<h3 style='margin-bottom: 0;'>3. Look Behind Assertions</h3><h4 style='margin-top: 0;'> allows you to match sections of a string by setting a condition the preceding characters:</h4>";
		...
		output +=  "<h3 style='margin-bottom: 0;'>4. Unicode Property Excapes</h3><h4 style='margin-top: 0;'>allows you to match characters using their Unicode character properties.</h4>";
		var unicodeString = "!@#$%^&*";
		output += "<ul>";
