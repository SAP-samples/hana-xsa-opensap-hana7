...
describe(__filename, () => {
	...
	beforeEach(async(done) => {
		...
		this.sp = await this.test.getStoredProc(this.db, "calculate_products");
		done();
	});

	it("Value Test", async(done) => {
		try {
			let results = await this.db.callProcedurePromisified(this.sp, {IM_PRODUCT_FILTER_STRING: "PRODUCTID = 'HT-1000'"});
			expect(results.results.length).not.toBeLessThan(16);
